...
import { Author, Course, CourseDTO } from "./course-info";
...
export class CoursesStoreService {
  ...
  private handleCourses(courses: CourseDTO[]): Observable<any[]> {
    if (courses.length > 0) {
      return forkJoin(
  ...
  getCourse(courseId: string) {
    this.coursesService
      ...
      .pipe(
        switchMap((course) => {
          if (course.authors && Array.isArray(course.authors) && course.authors.length > 0) {
            return forkJoin(
              (course.authors as string[]).map((authorId: string) =>
                this.coursesService.getAuthorById(authorId)
              )
            ).pipe(
              map((authors: Author[]) => {
